...
\section{Background, rationale, modeling strategy}
	...
	\subsection{Torque model}
		The motor's electrical behavior can be represented using two equivalent circuits using the Park transformation \cite{Park1929}. The result of the transformation for a 3-phase permanent-magnet synchronous motor (PMSM) is the \textit{dq} motor model. Figure \todo{Add dq equivalent-circuit diagram} shows the constant-parameter \textit{dq} equivalent circuits.
		
		\missingfigure{dq model equivalent circuits}
		
		The motor parameters are assumed to be constant with respect to stator currents and temperatures.
		
		By the equivalent circuits in Figure \todo{dq equiv ckt}, the stator voltages $V_d$ and $V_q$ can be found:
